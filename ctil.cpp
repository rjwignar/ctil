  ...
  while (found != std::string::npos) {
    ...
    if (end != std::string::npos) {
      std::string word_to_replace =
          "<i>" + input.substr(found + 1, end - found - 1) + "</i>";
...
void ctil::codeblockUpdate(std::string &input) {
  ...
  while (found != std::string::npos) {
    ...
    if (end != std::string::npos) {
      std::string blockToReplace =
          "<code>" + input.substr(found + 1, end - found - 1) + "</code>";
...
void ctil::generateHTML_txt(std::ifstream &infile, std::ofstream &outfile,
                            std::string filename) {
  ...
  while (infile && !infile.eof()) {
    ...
    // if line is non-empty and newlineCounter == 1, then wrap  content in
    // <p>...</p>, reset content to empty string, add line to content and keep
    // going
    {
      // pass what's currently in content to outfile, wrapped in paragraph tags
      // outfile << "<p>" + content + "</p>" << std::endl;

      // pass what's currently in content to bodyContent, wrapped in paragraph
      // tags
      bodyContent += "<p>" + content + "</p>" + "\n";
    ...
    // if line is non-empty and newlineCounter == 2, then follow title
    // adjustment logic
    {
      titleParsed = true;
      title = content;
  ...
  outfile
      ...
      << "<head>" << std::endl
      << "<link rel='stylesheet' "
         "href='https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.9.0/"
...
void ctil::generateHTML_md(std::ifstream &infile, std::ofstream &outfile,
                           std::string filename) {
  bool titleParsed = false;
  std::string title = filename;
  std::string line;
