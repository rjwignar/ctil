// ctil.cpp : This file contains the 'main' function. Program execution begins
// and ends there.
//
...
int main(int argc, char *argv[]) {
  ...
  if (std::filesystem::is_directory(argv[argc - 1])) {
    ...
    for (const std::filesystem::directory_entry &item :
      ...
      if (!(cdot::ctil::has_txt_suffix(itemPath) ||
            cdot::ctil::has_md_suffix(itemPath))) {
  ...
  } else // logic for single text file
  {
    ...
    if (cdot::ctil::has_txt_suffix(argv[1])) {
      newName = ((std::string)argv[1])
                    .substr(0, ((std::string)argv[1]).find(txt_suffix)) +
                ".html";
    } else if (cdot::ctil::has_md_suffix(argv[1])) {
      newName = ((std::string)argv[1])
                    .substr(0, ((std::string)argv[1]).find(md_suffix)) +
                ".html";
    }
    // std::cout << "new filename is: " + newName;
